...
def main(args):
    ...
    if args.model == "bigram":
        model = BigramLanguageModel(
            dataset_loader.vocab_size, embedding_dim=args.embedding_dim
        )
    elif args.model == "gpt":
    ...
    for iter in range(args.num_iterations):
        ...
        if iter % 1000 == 0:
            estimated_loss = estimate_loss(
                model, dataset_loader, args.batch_size, args.block_size, 100
            )
            logger.info(
                f"Iteration {iter} train loss: {estimated_loss['train']:.4f}, val loss: {estimated_loss['val']:.4f}"
            )
    generated_text = model.generate(
        torch.zeros((1, args.context_length), dtype=torch.long), max_new_tokens=100
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, choices=["bigram", "gpt"], default="bigram"
    )
